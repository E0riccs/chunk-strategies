"""
    对于文本格式的结构内容，提供转换为结构化数据的工具
"""

import json
import re

def _trans_to_qa_list(llm_answer: str):
    """
    args:
...
def _trans_to_score_dict(llm_answer: str):
    # try to find the first '{' and last '}'
    start_bracket = llm_answer.find('{')
    end_bracket = llm_answer.rfind('}')
    ...
    try:
        ...
        # Attempt to parse the extracted string
        data = json.loads(json_str)
        if isinstance(data, dict) and 'score' in data and 'justification' in data:
            return {
                'score': data['score'],
                'justification': data['justification']
            }
        return {} # Return empty dict if not a valid score result
    except json.JSONDecodeError:
        # Fallback: try to find score and justification using regex
        try:
            # Look for patterns like "score": number and "justification": "text"
            score_pattern = re.compile(r'"score"\s*:\s*(\d+(\.\d+)?)', re.DOTALL)
            justification_pattern = re.compile(r'"justification"\s*:\s*"(.*?)"', re.DOTALL)
            
            score_match = score_pattern.search(llm_answer)
            justification_match = justification_pattern.search(llm_answer)
...
def extract_qa_pairs(llm_answer):
    ...
    qa_list = _trans_to_qa_list(llm_answer)
...
def extract_score_res(llm_answer:str):
    """
    args:
        llm_answer: str, the answer from llm
    Extracts score and reason from text llm response.

    Returns:
        flag: bool, whether the answer conclude a valid score result
        dict: A dictionary containing the score and justification.

    Example:
    '{
        "score": 1,
        "justification": "It is OK but not good"
    }'
    """

    score_dict = _trans_to_score_dict(llm_answer)
    ...
    if 'score' not in score_dict:
        return False,{}
    if not isinstance(score_dict['score'], int):
        return False,{}
    if score_dict['score'] not in {0,1,2}: # 根据 Prompt 定义
        return False,{}
...
# '''
#     qa_pairs = _trans_to_qa_list(text_dict)
#     print(qa_pairs)

    text_dict = '''
    '\n{\n  "score": 1,\n  "justification": "模型答案虽合理但存在推测性（如\'心理负担\'未直接文本支撑），且遗漏关键点：①未提及\'倒计时终点\'涉及三体组织核心战略（标准答1）；②未指出叶文洁刻意转移话题至纳米材料（标准答3）。仅部分涵盖标准答2（涉及机密）但表述模糊。"\n}'
    '''

    flag, res = extract_score_res(text_dict)
    print(res)